# train_final_torch_ram.py
# (RAM Pre-loading 적용 버전 - 속도 극대화)
...
config = SimpleNamespace(
    ...
    learning_rate = 0.0009768995595899274, 
    rnn_model = "LSTM",
    rnn_units = 64,
    epochs = 100
)
...
class MultiModalRamDataset(Dataset):
    def __init__(self, df, npy_dir, png_dir, img_dims, npy_dims, scaler, mode='Train'):
        ...
        # [RAM 캐싱] 데이터를 리스트에 미리 저장
        self.cached_data = []
        
        print(f"[{mode}] 데이터를 RAM에 미리 로드하는 중... (약 1~2분 소요)")
        for i in tqdm(range(len(self.df)), desc=f"Loading {mode} data"):
...
# --- 5. Early Stopping (동일) ---
class EarlyStopping:
    def __init__(self, patience=10, verbose=False, delta=0, path='best_multimodal_ae_torch_ram.pt'):
    ...
    def save_checkpoint(self, val_loss, model):
        if self.verbose: print(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}). Saving...')
...
if __name__ == "__main__":
    print(f"🚀 현재 디바이스: {device} (RAM Pre-loading Mode)")
    wandb.init(project="deepfake-multimodal-AE", name="Torch_RAM_Optimized", config=config.__dict__)
    
    if not os.path.exists(CSV_FILE_PATH): exit()
    ...
    # ★ 여기서 데이터를 RAM에 올립니다 (초반에 시간 조금 걸림)
    ...
    # num_workers=0 으로 설정해도 빠릅니다 (이미 RAM에 있으니까!)
    ...
    print(f"\n🚀 초고속 학습 시작 (Batch: {config.batch_size})...")
    for epoch in range(config.epochs):
        ...
        wandb.log({"epoch": epoch+1, "loss": avg_train, "val_loss": avg_val})
