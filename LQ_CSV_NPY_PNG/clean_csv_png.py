...
def synchronize_images_with_confirmation(csv_path, folder_path, id_column='video_id'):
    """
    CSV의 ID 컬럼에 없는 PNG 파일들을 식별하고, 
    ...
    if not os.path.exists(folder_path):
        print(f"[에러] 이미지 폴더를 찾을 수 없습니다: {folder_path}")
        return
    ...
    print(f"\n[검사 중] '{folder_path}' 폴더를 스캔하고 있습니다...")
    ...
    count_total = len([f for f in files if f.lower().endswith('.png')])
    count_delete = len(files_to_delete)

    # 4. 결과 요약 및 사용자 확인
    print("-" * 60)
    print(f"검사 결과:")
    print(f" - 폴더 내 전체 PNG 파일: {count_total}개")
    ...
    # 5. 사용자 입력 대기 (y/n)
    user_input = input(f"정말로 위 {count_delete}개의 파일을 삭제하시겠습니까? (y/n): ").strip().lower()
    ...
    if user_input == 'y':
        ...
        for file_name in files_to_delete:
            ...
            try:
                os.remove(full_path)
                deleted_count += 1
                # 너무 많은 로그 출력을 방지하기 위해 주석 처리 (필요시 해제)
...
# 1. CSV 파일 경로
target_csv_path = 'model_CNN_train_CLEAN.csv'

# 2. 이미지가 있는 폴더 경로
target_image_folder = '3_audio_spectrograms'
...
if __name__ == "__main__":
    synchronize_images_with_confirmation(target_csv_path, target_image_folder, target_column_name)